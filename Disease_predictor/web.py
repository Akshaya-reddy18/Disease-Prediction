...
pcods_model = pickle.load(open(r"MODELS/pcod_model.sav", 'rb'))
endometriosis_model = pickle.load(
    open(r"MODELS/endometriosis_model.sav", 'rb'))

with st.sidebar:
    selected = option_menu("Disease Prediction System",
                           ["Diabetes Prediction", "Heart Disease Prediction",
                               "Parkinson's Prediction", "PCOD Prediction", "Endometriosis Prediction"],
                           menu_icon="hospital-fill",  icons=["activity", "heart", "person", "gender-female", "file-medical"], default_index=0)
...
Age, BMI, MenstrualCycleLength, MenstrualFlowIntensity, PelvicPainScore, InfertilityHistory, FamilyHistory, BiomarkerLevel = None, None, None, None, None, None, None, None
...
# Parkinson's Prediction Page
...
elif selected == "Endometriosis Prediction":
    st.title('Endometriosis Disease Prediction using Machine Learning')
    col1, col2, col3 = st.columns(3)
    with col1:
        Age = st.text_input('Patient age')
    with col2:
    ...
    with col3:
        MenstrualCycleLength = st.text_input('Menstrual Cycle Length')
    with col1:
        MenstrualFlowIntensity = st.text_input('Menstrual Flow Intensity')
    with col2:
        PelvicPainScore = st.text_input('Pelvic Pain Score')
    with col3:
        InfertilityHistory = st.text_input('Infertility History')
    with col1:
        FamilyHistory = st.text_input('Family History')
    with col2:
        BiomarkerLevel = st.text_input('Biomarker Level')

        Endometriosis_diagnosis = ''
    if st.button('Endometriosis Test Result'):
        user_input = [Age, BMI, MenstrualCycleLength, MenstrualFlowIntensity,
                      PelvicPainScore, InfertilityHistory, FamilyHistory, BiomarkerLevel]
        user_input = [float(x) for x in user_input]
        Endometriosis_prediction = endometriosis_model.predict([user_input])
        if Endometriosis_prediction[0] == 1:
            Endometriosis_diagnosis = 'The person is battling Endometriosis disease.'
        else:
            Endometriosis_diagnosis = 'The person is free from Endometriosis disease.'
        st.success(Endometriosis_diagnosis)
